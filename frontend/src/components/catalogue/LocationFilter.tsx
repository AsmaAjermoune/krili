...
export default function LocationFilter({ ville, rayon, lat, lng, onApply }: LocationFilterProps) {
  ...
  // Draft state — local to the open popover
  ...
  // Nominatim results for cities not in our static list
  ...
  // When GPS coordinates arrive, update draft and find nearest city label
  useEffect(() => {
  ...
  // Static list filtered by search term (shown when search is short)
  ...
  // Show Nominatim results if search is long enough AND static results are empty
  ...
  function handlePositionChange(newLat: number, newLng: number) {
    setDraftLat(newLat);
    setDraftLng(newLng);
    // Snap label to nearest city so the trigger shows something meaningful
    const nearest = findNearestCity(newLat, newLng);
    const dist = haversineKm({ lat: newLat, lng: newLng, name: "" }, nearest);
    // Only snap to city name if within 80 km, otherwise keep as "Position choisie"
  ...
  // Apply is always valid — the user explicitly opened the filter and the map shows a real position
          ...
          <div className="min-w-0">
            {ville ? (
              ...
              <div className="flex items-center gap-2 min-w-0">
                {/* Mini Morocco map preview so the user understands what this filter does */}
                <img
                  src="/ma-orange.svg"
                  alt=""
                  aria-hidden="true"
        ...
        <div className="flex flex-col gap-4 p-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Slider
              ...
              className="[&_[role=slider]]:bg-[#ff6700] [&_[role=slider]]:border-[#ff6700]"
            />
